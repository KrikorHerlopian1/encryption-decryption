...
loopEncrypt:
	...
	@ check for uppercase, in case we confirm its not encrypt,
	@ call checkdecrypt to check if its decrypt

	@ we have to check for upper case as well. in case we confirm its
	@ not encrypt, call checkdecrypt to check if its decrypt

checkEncryptCaseInsensitive:	@ add 32
...
loopDecrypt:
	...
	@ check for uppercase letter as well. D or d. If we get to confirm
	@ its not decrypt , end program.

checkDecryptCaseInsensitive:
	add	R6, R6,#32	@ add 32, lowecase the character 
...
asIs:
	...
	@ 6 * key length * key length for inbuff and outbuff

inputUser:
	mul	R11, R4, R4
	mov	R9, #6
	mul	R11, R11, R9	@ allocating space key*key*6  (96b if key=4)
	sub	SP, SP, R11
	mov	R9, #0		@ key index
...
@ R2 to tell whether to do encrypt(0) or decrypt(1)
	...
	@ this will do encryption or decryption using vigenere,
	@ result will return in R0

	bl	vigenere
	
 	mov	R3, R0	@ moving result to R3
	mov 	R0, R5	@ moving key to R0
	mov	R5, #0
	mov	R8, #0
	mov	R12, #0
	mov	R11, #0
	...
	@ R1 is the length of the plain/ciphered text
	@ R2 to tell whether to do encrypt(0) or decrypt(1)
	@ R3 is text to encrypt/decrypt
	@ R4 is key length	
	bl	transposition
	b	printFinal
	
transpositionThanVigenere:
	mov	R5,#0
	mov	R8,#0
	mov	R12, R1
	mov	R12, #0
...
@ since in transposition we add to the key 100, let us move it back to old value.
@ So we get correct key back, to do vigenere cipher.
...
finalVig:
	...
	mov	R11, #0
	push	{R6}
	@ this will do encryption or decryption using vigenere, result will
	@ return in R0
	bl	vigenere
	b	printFinal
...
printFinal: 
	...
	mov	R0, #1
	mov	R2, R6		@ moving length to print into R2.
...
compare10:
	...
	ldrb	R10, [R0,R5]
	add	R10, #100
	strb	R10, [R0,R5]	@ update key we just used by adding 200,
				@ to find next lowest key easily.
	mov	R8, #0
	add	R6, #1
	mov	R5, #0
...
@ lowest easier. Lowest column key to be in R8. Once we determined lowest
@ column , we go to setup9.
...
@ We want to loop, and add to our result. So assuming column 4 determined to
@ be lowest, we add column 4 results into r11. We go over the column by
@ adding key length. So first R8 is 3 ( column 4), next loop we want text at
@ index 9 (if key length is 6), next loop we want letter at index 15... so on
...
@ we add column first key letter into column 4.
...
@ in case new line is last letter, i am handling scenario where new line
@ is added in middle of result.
...
vigenere:
	...
	@ we start process of loop over every character to encrypt or decrypt
	@ it. R1 is length of plain/ciphered text.R7 is index of every
	@ character we passing on for plain/ciphered text.
...
loopPlainText:
	...
	sub	R8, R5, #32	@ else subtract another 32 for lowercase scenario
